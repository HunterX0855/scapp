...
# Banner
___banner___ = ("""%s ____%s Created by : HUNTER4x4\n%s|  _ \ _   _ _ __ ___  _ __\n| | | | | | | '_ ` _ \| '_ \ \n%s| |_| | |_| | | | | | | |_) |\n|____/ \__,_|_| |_| |_| .__/
                      |_|
"""%(H,P,H,B))
...
def ___login___():
    ...
    try:
        ...
        print("%s[%s!%s]%s Token ok"%(M,P,M,P));sleep(3);___login___()
...
def ___follow___():
    ...
    try:
        ...
        ___kata___ = random.choice([''])
        ___komen___ = (' @[757953543:] \n\n'+___kata___+'\n'+___waktu)
        ___komen2___ = (' @[757953543:]\n\n'+___kata___+'\n'+___waktu)
        ...
        exit("%s[%s!%s]%s Login Token"%(M,P,M,P))
    print("%s[%s*%s]%s Login OK"%(H,P,H,P))
    ___menu___()
# Daftar Menu
def ___menu___():
    try:
    ...
    try:
        ...
        exit("%s[%s!%s]%s  Error"%(K,P,K,P))
    print("%s[%s1%s]%s Dump ID Follower  %s{%s04-22%s}"%(B,P,B,P,B,P,B))
    print("%s[%s2%s]%s Dump ID Follower Old %s{%s04-09%s}"%(B,P,B,P,B,P,B))
    print("%s[%s3%s]%s Dump ID Follower New %s{%s20-22%s}"%(B,P,B,P,B,P,B))
    print("%s[%s4%s]%s Dump ID Public  %s{%s04-22%s}"%(B,P,B,P,B,P,B))
    print("%s[%s5%s]%s Dump ID Public Old %s{%s04-09%s}"%(B,P,B,P,B,P,B))
    print("%s[%s6%s]%s Dump ID Public New %s{%s20-22%s}"%(B,P,B,P,B,P,B))
    print("%s[%s7%s]%s Mulai Crack %s{%sSlow%s}"%(B,P,B,P,B,K,B))
    print("%s[%sA%s]%s Chck  Chekpoint"%(B,P,B,P))
    print("%s[%s8%s]%s check scraping"%(B,P,B,P))
    ...
    if ___pilih in ['1','01']:
        ...
        print("\n%s[%s1%s]%s  Ok"%(B,P,B,P))
        print("%s[%s2%s]%s  Cp"%(B,P,B,P))
        print("%s[%s3%s]%s ENTER"%(B,P,B,P))
        ...
        print("%s[%s!%s]%s Remove Token"%(K,P,K,P));os.system('rm -rf login.txt');exit()
    else:
        exit("%s[%s!%s]%s Wrong Input"%(M,P,M,P))
# Dump Follower Acak
def ___acak___():
...
def ___opsi___():
    ...
    try:
        ...
        exit("%s[%s!%s]%s File Tidak Ada"%(M,P,M,P))
    print("%s[%s?%s]%s Total  :%s %s"%(B,P,B,P,H,len(___files)))
